# -*- coding: utf-8 -*-
"""
Created on Fri May 21 11:01:11 2021

@author: https://github.com/liuhu-bigeye/enctc.crnn/blob/master/pytorch_ctc/ctc_ent.py
"""
import os
import torch as T
...
from modules.m_ctc import m_eye, log_batch_dot, log_sum_exp
...
def ctc_ent_cost(out, targets, sizes, target_sizes, blank=0, use_softmax=True, use_log=True, reduction='mean'):
...
#    param target_sizes: size for targets (N)
    ...
    if not cuda:
        H, costs = loss_func(pred.cpu(), sizes.data.type(longX), target, target_sizes.data.type(longX), blank)
    else:
        H, costs = loss_func(pred, sizes.data.type(longX), target, target_sizes.data.type(longX), blank)

    if reduction=='sum':
        return H.sum(), costs.sum()
    elif reduction=='mean':
        return H.sum()/batch, costs.sum()/batch
    else:
        return H, costs
...
def test_seg_ctc(use_mine=True, use_log=False):
    ...
    for i in range(50):
        if use_mine:
            ...
            logging.info('%d, cost: %s, entropy: %s'% (i, cost.data.item(), H.data.item()))
#            cost = 0.9*cost - 0.1*H
        else:
            from warpctc_pytorch import CTCLoss
            criterion = CTCLoss().cuda()
            cost = criterion(pred, token, sizes, target_sizes)
            logging.info('%d, cost: %s'% (i, cost.data.item()))
