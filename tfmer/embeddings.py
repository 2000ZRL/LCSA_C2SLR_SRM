...
class MaskedNorm(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor):
        '''
        mask: [B,1,max_len], True for padding values!
        '''
        if self.training:
            ...
            return scattered.reshape([x.shape[0], self.num_features, -1])
        else:
            reshaped = x.reshape([-1, self.num_features])
            batched_normed = self.norm(reshaped)
            return batched_normed.reshape([x.shape[0], self.num_features, -1])
