...
#cat final_phoenix_noPause_noCompound_lefthandtag_noClean.stm  >tmp.stm

cat ${workdir}phoenix2014-SI7-groundtruth-$partition.stm | sort  -k1,1 > ${workdir}$tmp_prefix.tmp.stm

#add missing entries, so that sclite can generate alignment
python ./${workdir}mergectmstm_SI.py ${workdir}$tmp_prefix.tmp2.ctm ${workdir}$tmp_prefix.tmp.stm 
