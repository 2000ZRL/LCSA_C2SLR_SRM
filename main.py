...
def parse_args():
    ...
    p.add_argument('--sema_cons', type=str, default=None, choices=[None, 'drop', 'shuffle', 'drop_shuffle'])
    p.add_argument('--pl', type=int, default=0, choices=[0,1], help='pseudo labeling')
    p.add_argument('--dcn_ver', type=str, default='v2', choices=['v1', 'v2'])
    ...
    p.add_argument('--cbam_pool', type=str, default='max_avg', choices=['max_avg', 'softmax', 'max_softmax'], help='channel pool type of CBAM')
    p.add_argument('--cbam_no_channel', type=int, default=0, choices=[0,1], help='whether use channel gate in CBAM')
    p.add_argument('--att_sup_type', type=str, default='first', choices=['first', 'all', 'res'])
    p.add_argument('--dcn_lr_factor', type=float, default=1.0)
    ...
    p.add_argument('--mode', type=str, default='train', choices=['train', 'test', 'vis_cam', 'vis_att', 'vis_badcase'])
    p.add_argument('--test_split', type=str, default='test', choices=['test', 'dev', 'train_no_drop'])  #maybe we want to test on training set
    ...
    #pose
    p.add_argument('--pose', type=str, default=None, choices=[None, 'filter', 'modality', \
                                                              'deform', 'deform_all', 'deform_mask', 'deform_patch', 'deform_and_mask', 'super_att'])
...
if __name__ == '__main__':
    ...
    os.system("cp semi.py {:s}".format(args.save_dir))
    ...
    dset_dict = {'2014': {'cls': PhoenixVideoTextDataset, 'root': '../../data/phoenix2014-release/phoenix-2014-multisigner', 'mean': [0.5372,0.5273,0.5195], 'hmap_mean': [0.0236, 0.0250, 0.0164, 0.0283, 0.0305, 0.0240, 0.0564]},
                 '2014T': {'cls': PhoenixTVideoTextDataset, 'root': '../../data/PHOENIX-2014-T-release-v3/PHOENIX-2014-T', 'mean': [0.5372,0.5273,0.5195]},
                 '2014SI': {'cls': PhoenixSIVideoTextDataset, 'root': '../../data/phoenix2014-release/phoenix-2014-signerindependent-SI5', 'mean': [0.5405, 0.5306, 0.5235]},
                 'csl1': {'cls': CSLVideoTextDataset, 'root': ('../../data/ustc-csl', 'split_1.txt'), 'mean': [0.5827, 0.5742, 0.5768]},
                 'csl2': {'cls': CSLVideoTextDataset, 'root': ('../../data/ustc-csl', 'split_2.txt'), 'mean': [0.5827, 0.5742, 0.5768]},
    ...
    elif args.mode == 'vis_badcase':
        for fname in os.listdir(args.save_dir):
            if 'pkl' in fname and 'ep' in fname:
                model_file = os.path.join(args.save_dir, fname)
                training_manager.vis_badcase(model_file)
